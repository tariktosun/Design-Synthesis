...
T = times[2].values()
numnodes = numnodes = [(2**d)-1 for d in range(1,D+1)]
...
figure()
plot(numnodes, T, 'o-', label='Data')
plot(range(1,70), fitcurve, 'r', label='Cubic Poly')
title('Alternating Binary Tree: Run time as function of sub-tree nodes')
xlabel('Number of sub-tree nodes (depths 1-6)')
ylabel('Average compute time (seconds)')
